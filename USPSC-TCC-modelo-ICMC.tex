...
\usepackage{float}    % para o option [H]
\usepackage{caption}  % opcional, melhora controles de caption
\usepackage{tabularx}
...
%%%% !!!!!!!! \includepdf{pre_tex/folhadeaprovacao.pdf}
...
\include{pre_tex/folhadeaprovacao}


\includepdf{pre_tex/PaginaEmBranco.pdf}

% ---
% Dedicatória
% ---
\include{pre_tex/USPSC-Dedicatoria}
% ---

% ---
% Agradecimentos
% ---
\include{pre_tex/USPSC-Agradecimentos}
% ---
...
\begin{epigrafe}
    \vspace*{\fill}
	\begin{flushright}
	\textit{``Prefiro ter perguntas que não podem ser respondidas\\
				do que respostas que não podem ser questionadas.''\\
				Richard P. Feynman }
	\end{flushright}
\end{epigrafe}
% ---
...
% Resumo
% ---
\include{pre_tex/USPSC-Resumo}
% ---

% Abstract
% ---
\include{pre_tex/USPSC-Abstract}
% ---
...
% ---
\begin{siglas}
    \item[ABRINQ] Associação Brasileira de Fabricantes de Brinquedos
	\item[BC] Filtragem Baseada em Conteúdo
    \item[BGG] BoardGameGeek
    \item[BL] Modelo Baseline
    \item[CF] Filtragem Colaborativa (\textit{Collaborative Filtering})
    \item[DL] Aprendizagem Profunda (\textit{Deep Learning})
    ...
    \item[FC+BC] Modelo Híbrido: Filtragem Colaborativa + Conteúdo
    \item[FC+BL] Modelo Híbrido: Filtragem Colaborativa + Baseline
    \item[ICMC] Instituto de Ciências Matemáticas e de Computação
    \item[KNN] K-Vizinhos Mais Próximos (\textit{K-Nearest Neighbors})
    \item[MAE] Erro Absoluto Médio (\textit{Mean Absolute Error})
    \item[ML] Aprendizagem de Máquina (\textit{Machine Learning})
    \item[MIT] Instituto de Tecnologia de Massachusetts (\textit{Massachusetts Institute of Technology})
    \item[P@10] Precisão no Top-10 (\textit{Precision@10})
    \item[R@10] Revocação no Top-10 (\textit{Recall@10})
    \item[RMSE] Raiz do Erro Quadrático Médio (\textit{Root Mean Squared Error})
    \item[SR] Sistema de Recomendação
    \item[SRs] Sistemas de Recomendação
    \item[SVD] Decomposição em Valores Singulares (\textit{Singular Value Decomposition})
    \item[SGD] Gradiente Descendente Estocástico (\textit{Stochastic Gradient Descent})
    \item[SVM] Máquina de Vetores de Suporte (\textit{Support Vector Machine})
    \item[TF-IDF] Frequência do Termo - Frequência Inversa do Documento (\textit{Term Frequency - Inverse Document Frequency})
\end{siglas}
...
% ---
\begin{simbolos}
    \item[$\beta$] Coeficiente de hibridização do modelo híbrido
    \item[$\mu$] Média global das avaliações
    \item[$b_u$] Viés associado ao usuário $u$
    \item[$b_i$] Viés associado ao item $i$
    \item[$R$] Matriz de avaliações usuário--item
    \item[$\mathbb{R}^{m \times n}$] Espaço das matrizes reais com $m$ linhas e $n$ colunas
    \item[\( \mathbb{R}^n \)] Espaço dos vetores reais com $n$ dimensões
	\item[$sim(u,v)$] Similaridade entre os usuários $u$ e $v$
	\item[$sim(i,j)$] Similaridade entre os itens $i$ e $j$
    \item[$r_{ui}$] Avaliação do usuário $u$ para o item $i$
    \item[$\hat{r}_{ui}$] Avaliação prevista para o usuário $u$ e item $i$
    \item[$m$] Número total de usuários
    \item[$n$] Número total de itens
    \item[$k$] Número de vizinhos considerados no modelo
    \item[$N(u)$] Conjunto de vizinhos do usuário $u$
    \item[$I_u$] Conjunto de itens avaliados pelo usuário $u$
    \item[$\mathcal{K}$] Conjunto de avaliações conhecidas 
    \item[$O(m^2)$] Complexidade computacional quadrática
    \item[$O(m)$] Complexidade computacional linear
    \item[\(\hat{r}_{u,i}^{SR_1}\) e \(\hat{r}_{u,i}^{SR_2}\)] Avaliações previstas pelos Sistemas de Recomendação 1 e 2, respectivamente
    \item[$\hat{r}_{u,i}^{H}$] Avaliação prevista pelo modelo híbrido
...
\chapter{Dados e Variáveis}
\label{chapter:dados}
\input{tex/base_dados.tex}

\chapter{Desenvolvimento e Avaliação do Sistema de Recomendação}
\label{chapter:pratica}
\input{tex/parte_pratica.tex}
...
% Apêndices
% ----------------------------------------------------------
\include{pos_tex/USPSC-Apendices}
