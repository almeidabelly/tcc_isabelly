...
\textbf{Vantagens}
\begin{itemize}
  \item Menor impacto do \textit{cold-start} de usuários
\end{itemize}
...
\hat{r}_{ui}^{(h)} =
\alpha\,\hat{r}_{ui}^{(FC)} +
(1-\alpha)\,\hat{r}_{ui}^{(BC)}
\]

\vspace{0.3cm}

\begin{itemize}
  \item Integra filtragem colaborativa e conteúdo
...
As métricas de avaliação foram divididas em dois grupos:
...
\textbf{Métricas de Ranqueamento}
\[
\text{Precision@k} =
\frac{| \text{itens relevantes} \cap \text{top-}k |}{k}
\]
